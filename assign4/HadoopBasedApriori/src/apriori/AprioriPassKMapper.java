...
public class AprioriPassKMapper extends Mapper<LongWritable, Text, Text, IntWritable>
{
    ...
    public void setup(Context context)
        ...
        System.out.println("[DEBUG] For passNum : " + passNum + " CandNum : " + candidateItemSets.size());
...
//    2) Emit the above itemsets as (itemset, 1).
    ...
    public void map(LongWritable key, Text txnRecord, Context context)
        ...
        for(int i = 0; i < matchedItemSet.size(); i++) {
            Text newItem = new Text();
            newItem.set(matchedItemSet.get(i).toString());
            context.write(newItem,one);
        }
    }
}
