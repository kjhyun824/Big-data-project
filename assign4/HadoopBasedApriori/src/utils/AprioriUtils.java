...
public class AprioriUtils {
    ...
    public static boolean hasMinSupport(double minSup, int numTxns, int itemCount) {
        ...
        double threshold = minSup * (double) numTxns;
...
*/
    ...
    public static List<ItemSet> getCandidateItemSets(List<ItemSet> prevPassItemSets, int itemSetSize) {
        ...
        Map<Integer, ItemSet> itemSetMap = generateItemSetMap(prevPassItemSets); // <HashCode , ItemSet>
        Collections.sort(prevPassItemSets);
        int prevPassItemSetsSize = prevPassItemSets.size();

        if(prevPassItemSetsSize < itemSetSize) {
        ...
        while (true) {
            if (back < prevPassItemSetsSize && needGenerate(prevPassItemSets.get(front), prevPassItemSets.get(back))) {
                ItemSet newItem = new ItemSet();

                for (int i = 0; i < itemSetSize; i++) {
                    newItem.add(prevPassItemSets.get(front).get(i));
                }

                newItem.add(prevPassItemSets.get(back).get(itemSetSize - 1));

                if (!prune(itemSetMap, newItem)) {
            ...
            if (front >= (prevPassItemSetsSize-1)) break;
        }

        return candidateItemSets;
    }

    static boolean needGenerate(ItemSet front, ItemSet back) {
        if (front.size() == 1) {
            return true;
        }
    ...
    static List<ItemSet> getSubSets(ItemSet itemSet) {
        ...
        for (int i = 0; i < itemSetSize; i++) {
            for (int j = 0; j < itemSetSize; j++) {
                if (i != j) {
                    subSets.get(i).add(itemSet.get(j));
                }
            }
        }
